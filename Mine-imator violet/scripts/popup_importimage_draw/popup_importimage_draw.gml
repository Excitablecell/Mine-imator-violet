...
if (draw_button_normal("importimageok", dx, dy, dw, 32))
{
	...
	{
		if(popup_importimage.filenameArray_index>1){
			for (var i = 0; i < popup_importimage.filenameArray_index;i+=1){
				action_res_image_load(popup_importimage.filenameArray[i], popup.type)
				}
		}else{
			action_res_image_load(popup.filename, popup.type);
		}
		popup_importimage.filenameArray_index = 0;
		popup_close()
	}
}
