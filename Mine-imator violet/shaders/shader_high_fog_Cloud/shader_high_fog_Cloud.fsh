...
vec4 getVolumeFog(vec3 worldPosition, vec3 cameraPosition,vec3 fogRange)
{
   ...
   point += direction *(abs(cameraPosition.z - fogRange.y) / abs(direction.z));
	...
	for(int i=0;i<500;i++)
	{
		...
		/*float noise_use = noise((point+uOff)/40.0);         //这里对point.xy加减，可以偏移噪声图像，影响云的分布，例子：noise2D((point.xy+uOff)/50.0)
		noise_use += noise((point+uOff)/40.0*3.5)/3.5;
		noise_use += noise((point+uOff)/40.0*12.25)/12.25;
		noise_use += noise((point+uOff)/40.0*42.87)/42.87;
		noise_use /= 1.4472;
		noise_use *= weight;*/
		
		float noise_use = getCloudNoise(point+uOff/10.0);
