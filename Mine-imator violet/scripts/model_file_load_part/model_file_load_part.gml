...
with (new(obj_model_part))
{
	...
	if (!is_undefined(map[?"bend"]))
	{
		...
		// if (bend_inherit) //mb export may cause the first-level component to have inherit_bend attribute to cause an error, wait for mb to be repaired, and then restore this line of judgment
		if (bend_inherit && variable_instance_exists(other, "bend_inherit_angle"))
