...
if (background_fog_show && checkbox_expand_background_fog)
{
	...
	capwid = text_caption_width("backgroundfogdistance", "backgroundfogsize","backgroundfogheight","frameeditorcamerafogheight","frameeditorcamerafogheightend","frameeditorcamerafogheightdensity","backgroundfogvfogbottom","backgroundfogvfogtop",
								"backgroundfogvfogtop","backgroundfogvfogwide","backgroundfogvfogthreshold","backgroundfogvfogdensity","backgroundfogvfogpositionx","backgroundfogfogoffx","backgroundfogvfogsize")
	...
	// Fog height
	tab_control_dragger()
	draw_dragger("backgroundfogheight", dx, dy, dw, background_fog_height, background_fog_height / 100, 10, 2000, 1000, 10, tab.background.tbx_fog_height, action_background_fog_height)
	tab_next()
	
	draw_label(text_get("backgroundfogheightfoglabel") + ":", dx, dy+5, fa_left, fa_middle)
	
	// Fog start height
	tab_control_meter()
	draw_meter("backgroundfogheightstart", dx+20, dy+12, dw, round(background_fog_start), 100, -3000, 3000, 0, 1, tab.background.tbx_fog_height_start, action_tl_frame_cam_fog)
	tab_next()
	
	// Fog end height
	tab_control_meter()
	draw_meter("backgroundfogheightend", dx+20, dy+6, dw, round(background_fog_end), 100, -3000, 3000, 0, 1, tab.background.tbx_fog_height_end, action_tl_frame_cam_fog_end)
	tab_next()
	
	// Fog density
	tab_control_meter()
	draw_meter("backgroundfogheightdensity", dx+20, dy, dw, background_fog_density, 100, 0, 2, 0, 0.01, tab.background.tbx_fog_height_density, action_tl_frame_cam_fog_density)
	tab_next()
	
	draw_label(text_get("backgroundfogvolumefoglabel") + ":", dx, dy+5, fa_left, fa_middle)
	
	// Volume Fog bottom height
	tab_control_meter()
	draw_meter("backgroundfogvfogbottom", dx+20, dy+18, dw, v_fog_bottom, 100, -1000, 0, -1, 1, tab.background.tbx_v_fog_bottom, action_v_fog_bottom)
	tab_next()
	
	// Volume Fog top height
	tab_control_meter()
	draw_meter("backgroundfogvfogtop", dx+20, dy+12, dw, v_fog_top, 100, 0, 1000, 50, 1, tab.background.tbx_v_fog_top, action_v_fog_top)
	tab_next()
	
	// Volume Fog wide
	tab_control_meter()
	draw_meter("backgroundfogvfogwide", dx+20, dy+6, dw, v_fog_wide, 100, 100, 1000, 100, 5, tab.background.tbx_v_fog_wide, action_v_fog_wide)
	tab_next()
	
	// Fog noise threshold
	tab_control_meter()
	draw_meter("backgroundfogvfogthreshold", dx+20, dy, dw, v_fog_threshold, 70, 0, 1, 0, 0.01, tab.background.tbx_v_fog_threshold, action_v_fog_threshold)
	tab_next()
	
	// Fog noise size
	tab_control_meter()
	draw_meter("backgroundfogvfogsize", dx+20, dy, dw, v_fog_size, 70, 0.1, 100, 40, 0.01, tab.background.tbx_v_fog_size, action_v_fog_noiseSize)
	tab_next()
	
	// Fog Density (Volume) <- Stanwade from here
	tab_control_meter()
	draw_meter("backgroundfogvfogdensity", dx+20, dy, dw, v_fog_density, 70, 0, 1, 0, 0.01, tab.background.tbx_v_fog_density, action_background_vfog_density)
	tab_next()
	
	// Fog Position X (Volume)
	tab_control_dragger()
	draw_dragger("backgroundfogvfogpositionx", dx+20, dy, dw, v_fog_pos_x, 10, -30000, 30000, 0, 1, tab.background.tbx_v_fog_pos_x, action_background_vfog_pos_x)
	tab_next()
	
	// Fog Position Y (Volume)
	tab_control_dragger()
	draw_dragger("backgroundfogvfogpositiony", dx+20, dy, dw, v_fog_pos_y, 10, -30000, 30000, 0, 1, tab.background.tbx_v_fog_pos_y, action_background_vfog_pos_y)
	tab_next()
	
	// Fog Position Z (Volume)
	tab_control_dragger()
	draw_dragger("backgroundfogvfogpositionz", dx+20, dy, dw, v_fog_pos_z, 10, -30000, 30000, 0, 1, tab.background.tbx_v_fog_pos_z, action_background_vfog_pos_z)
	tab_next()
	
	// Fog OFF X (Volume)
	tab_control_dragger()
	draw_dragger("backgroundfogfogoffx", dx+20, dy, dw, fog_off_x, 10, -1000, 1000, 0, 1, tab.background.tbx_fog_off_x, action_background_fog_fogoff_x)
	tab_next()
	
	// Fog OFF Y (Volume)
	tab_control_dragger()
	draw_dragger("backgroundfogfogoffy", dx+20, dy, dw, fog_off_y, 10, -1000, 1000, 0, 1, tab.background.tbx_fog_off_y, action_background_fog_fogoff_y)
	tab_next()
	
	// Fog OFF Z (Volume)
	tab_control_dragger()
	draw_dragger("backgroundfogfogoffz", dx+20, dy, dw, fog_off_z, 10, -1000, 1000, 0, 1, tab.background.tbx_fog_off_z, action_background_fog_fogoff_z)
	tab_next()
